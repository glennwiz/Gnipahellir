...
// 16×22 ascii sprites, two walk frames each, shading baked into the art
...
player_pixel_color :: proc(p: ^Player, ch: rune) -> rl.Color {
    switch ch {
    case 'x': return rl.Color{13, 11, 19, 255}                         // outline
    case 'v': return rl.Color{16, 14, 22, 255}                         // void / black cloth
    case 'h': return shade_color(p.clothing_color, 0.40)               // clothing shadow
    case 'H': return shade_color(p.clothing_color, 0.72)               // clothing mid
    case 'L': return shade_color(p.clothing_color, 1.08)               // clothing rim light
    case 'y': return shade_color(p.hair_color, 0.55)                   // hair / beard shadow
    case 'Y': return p.hair_color                                      // hair / beard
    case 'k': return rl.Color{158, 116, 86, 255}                       // skin shadow
    case 'K': return rl.Color{214, 170, 132, 255}                      // skin
    case 'm': return rl.Color{58, 62, 78, 255}                         // steel shadow
    case 'M': return rl.Color{132, 138, 158, 255}                      // steel
    case 'N': return rl.Color{200, 206, 224, 255}                      // steel highlight
    case 'f': return rl.Color{74, 56, 36, 255}                         // fur shadow
    case 'F': return rl.Color{122, 94, 58, 255}                        // fur
    case 'b': return rl.Color{66, 44, 28, 255}                         // leather shadow
    case 'B': return rl.Color{110, 70, 40, 255}                        // leather / boots
    case 'w': return rl.Color{168, 158, 132, 255}                      // bone shadow
    case 'W': return rl.Color{228, 220, 192, 255}                      // bone / horn / beak
    case 's': return rl.Color{70, 68, 80, 255}                         // stone shadow
    case 'S': return rl.Color{118, 116, 128, 255}                      // stone
    case 'g': return rl.Color{186, 98, 32, 255}                        // core ember ring
    case 'G': return rl.Color{255, 176, 64, 255}                       // glowing core
    case 'E': return rl.Color{255, 214, 92, 255}                       // eye glow
    case 'O': return rl.Color{205, 235, 255, 255}                      // orb core
    case 'Q': return rl.Color{96, 156, 255, 255}                       // orb halo
    case 'T': return rl.Color{200, 158, 72, 255}                       // gold trim
...
draw_player :: proc(p: ^Player, form: Player_Form, step_visual_y: f32 = 0) {
    ...
    for row in 0 ..< FRAME_HEIGHT {
        for col in 0 ..< FRAME_WIDTH {
            ...
            rl.DrawRectangleRec({cx - 3.5*ps, cy - 3.5*ps, 7*ps, 7*ps}, {120, 180, 255, u8(28 * pulse)})
            rl.DrawRectangleRec({cx - 2.5*ps, cy - 2.5*ps, 5*ps, 5*ps}, {150, 200, 255, u8(55 * pulse)})
        }
    }
    ...
    held := held_tool(p)
    if held != .None {
        tps := ps * 2  // tools keep the old 8×11-era pixel size after the grid doubled
        hand_x := origin_x + total_w - tps * 2
        if p.facing < 0 { hand_x = origin_x + tps }
        hand_y := origin_y + tps * 6
        if held == .Pickaxe {
            ...
            draw_pickaxe(hand_x, hand_y, tps, deg)
        } else {  // a wand tier — shaft with a tier-colored tip
            rl.DrawRectangleRec({hand_x, hand_y, tps, tps * 3}, rl.Color{90, 60, 40, 255})
            rl.DrawRectangleRec({hand_x + tps, hand_y - tps, tps, tps}, item_table[held].color)
        }
    }
}
